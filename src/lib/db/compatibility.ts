...
export const WIRE_COMPATIBLE_ENGINES: readonly WireCompatibleEngine[] = [
  ...
  },
  {
    ...
    caveats: [
      ...
      "The agent grounds a stock install now: the column capture is one row per table rather than one row per column against a 200-row budget (B52). On a database with real hypertable data every chunk still appears as its own table in the inventory.",
    ],
  },
  {
    name: "YugabyteDB",
  ...
  },
  {
    ...
    caveats: [
      ...
      "The agent needs a least-privilege agent role to ground a run: connecting as the cluster's own gpadmin is refused because the execution profile reads that role as too broad. With that role the capture succeeds (B52).",
  ...
  },
  {
    ...
    caveats: [
      ...
      "The agent needs a least-privilege agent role to ground a run: the image's own postgres superuser is refused because the execution profile reads that role as too broad. With that role the capture succeeds (B52).",
